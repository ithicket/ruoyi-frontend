...
import {delJobLog, cleanJobLog, sendJobLogPage} from "@/api/monitor/jobLog"
...
async function getList() {
  loading.value = true
  const response = await sendJobLogPage(queryParams.value)
...
function handleDelete(row) {
  const tmp = row.id ? [row.id] : ids.value
