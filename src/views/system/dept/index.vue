<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5" v-if="hasPermission('system:dept:add')">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-if="hasPermission('system:dept:edit')">修改
          </el-button>
          <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)"
                     v-if="hasPermission('system:dept:add')">新增
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-if="scope.row.parentId !== 0 && hasPermission('system:dept:remove')">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
const data = reactive({
  ...
  rules: {
    ...
    phone: [{pattern: /^1[3-9][0-9]\d{8}$/, message: "请输入正确的手机号码", trigger: "blur"}]
  },
})
