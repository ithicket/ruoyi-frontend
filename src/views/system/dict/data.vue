      ...
      <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-if="hasPermission('system:dict:edit')"> 修改 </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-if="hasPermission('system:dict:remove')"> 删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {getData, delData, addData, updateData, pageData, sendDictDataExport} from "@/api/system/dict/data"
...
function getList() {
  loading.value = true
  pageData(queryParams.value).then(response => {
...
function handleDelete(row) {
  const tmp = row.dictCode ? [row.dictCode] : ids.value;
...
function handleExport() {
  sendDictDataExport(queryParams.value)
      .then(response => {
